...
import { CURRENT_COMPANY_SIGNATURE_AUTHORIZATION_VERSION, CURRENT_SIGNATURE_CONSENT_VERSION, LEGAL_TEXTS } from './legal-texts';
...
export class LegalService implements OnApplicationBootstrap {
  ...
  async currentCompanySignatureAuthorization(): Promise<LegalTextVersion> {
    const row = await this.prisma.legalTextVersion.findUnique({
      where: { kind_version: { kind: LegalTextKind.COMPANY_SIGNATURE_AUTHORIZATION, version: CURRENT_COMPANY_SIGNATURE_AUTHORIZATION_VERSION } },
    });
    if (!row) throw new Error('Texto de autorização de assinatura da empresa não registrado');
    return row;
  }
}
